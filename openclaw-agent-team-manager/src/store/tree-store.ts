...
import {
  ...
  detectProviderMode,
} from "@/services/openclaw-provider";
import {
  loadLayoutIndex,
  saveLayoutIndex,
  saveLayout,
...
function classifyFile(path: string): "agent" | "skill" | "settings" | "context" | null {
  ...
  // OpenClaw config
  if (p.endsWith("/openclaw.json")) return "settings";
  // OpenClaw/Claude skills (supports non-.claude skill roots)
  if (p.includes("/skills/") && p.endsWith("/SKILL.md")) return "skill";
...
export const useTreeStore = create<TreeStore>()((set, get) => ({
  ...
  async loadProject(path: string) {
    ...
    try {
      const providerMode = await detectProviderMode(path);
      ...
      const filePaths = await scanProject(path, providerMode);
      ...
      const ownerName = metadata?.owner.name ?? (providerMode === "openclaw" ? "SAI Prime" : "Kevin");
  ...
  async saveTreeMetadata() {
    ...
    const updated: TreeMetadata = {
      owner: metadata?.owner ?? { name: get().providerMode === "openclaw" ? "SAI Prime" : "Kevin", description: "" },
      hierarchy,
      positions,
  ...
  async deployPipeline(nodeId: string, options?: { skipLaunch?: boolean }) {
    ...
    const appSettings = await readAppSettings(join(home, ".openclaw"));
    if (!appSettings.enableLegacyClaudeDeploy) {
      throw new Error("Legacy Claude deploy is disabled. Enable it manually in Settings first.");
    }
...
`;
  ...
  async switchLayout(layoutId: string) {
    const { projectPath, currentLayoutId, providerMode } = get();
    if (!projectPath) return;
    ...
    const filePaths = await scanProject(projectPath, providerMode);
    const next = new Map<string, AuiNode>();

    // Root node with the target layout's owner info
    const ownerName = targetMeta.owner?.name ?? (providerMode === "openclaw" ? "SAI Prime" : "Kevin");
